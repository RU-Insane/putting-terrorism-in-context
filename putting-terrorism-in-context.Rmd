...
# Filter out the top 5 countries with the most number of kills
top_countries <- top_kills_by_country %>%
  arrange(group_by()) %>%
  slice_head(n = 5) %>%
  pull(country_txt) %>% 

# Create a new data frame that shows the top 10 states with the most kills for each of the top 5 countries
top_states <- kills_by_year_by_country %>%
  filter(country_txt %in% top_countries) %>%
  group_by(country_txt, provstate) %>%
  summarise(total_kills = sum(nkill)) %>%
  arrange(country_txt, desc(total_kills)) %>%
  group_by(country_txt) %>%
  slice_head(n = 10)

# Plot a stacked bar chart showing the distribution of kills across the top 10 states for each of the top 5 countries
ggplot(top_states, aes(x = factor(country_txt), y = total_kills, fill = provstate)) +
  geom_col(position = "stack") +
  scale_fill_viridis_d() +
  labs(title = "Distribution of kills across top 10 states for top 5 countries",
       x = "Country",
       y = "Total number of kills") +
  theme_bw()
